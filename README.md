...
[![OpenSSF Scorecard](https://api.securityscorecards.dev/projects/github.com/Data-Driven-Disaster-Resilience/D3R-AC/badge)](https://securityscorecards.dev/viewer/?uri=github.com/Data-Driven-Disaster-Resilience/D3R-AC)

The badge above is served by OSSF's own scorecard.dev, from the results
`.github/workflows/scorecard.yml` publishes — not generated by this repo,
so it can't be locally faked into showing a better score than the real,
independently-computed one. It measures supply-chain/repo hygiene (pinned
dependencies, branch protection, CI presence, vulnerability-disclosure
policy, etc.), not smart-contract correctness — see `SECURITY.md` for the
project's actual audit status, which this badge doesn't change.
